...
type Take = { id: string; versionNumber: number; fileUrl: string | null; localPath: string | null; originalFileName: string | null; mediaKind: string | null; status: string; isSelected: boolean; createdAt: string };
...
type FolderInputAttributes = { webkitdirectory: string; directory: string };
...
export function ReviewBoard({ projectId, shots }: { projectId: string; shots: Shot[] }) {
  ...
  function setChosenFiles(list: FileList | File[]) { const accepted = Array.from(list).filter((file) => /^image\//.test(file.type) || /\.(png|jpe?g|webp|gif)$/i.test(file.name)); setFiles(accepted); setReport(null); setMessage(accepted.length ? `${accepted.length} image(s) ready to match.` : null); setError(accepted.length ? null : "No supported images were found in that selection."); }
  ...
  async function upload() {
    if (!files.length) { setError("Choose, drop, or select a folder containing one or more images first."); return; }
  ...
  return <div className="flex flex-col gap-8">
    <section className={cardPadded}><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className={sectionLabel}>Bulk still-image intake</h2><p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Drop all ChatGPT stills together. Names such as shot_01.png, shot01_s1.png, and shot1,s2.png are matched automatically.</p></div><button type="button" disabled={busy !== null || !files.length} onClick={upload} className={buttonPrimary}>{busy === "upload" ? "Uploading…" : `Upload ${files.length || ""} image${files.length === 1 ? "" : "s"}`}</button></div>
      <div onDragEnter={(event) => { event.preventDefault(); setDragging(true); }} onDragOver={(event) => event.preventDefault()} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); setChosenFiles(event.dataTransfer.files); }} className={`mt-4 rounded-xl border-2 border-dashed p-7 text-center transition ${dragging ? "border-indigo-500 bg-indigo-50 dark:bg-indigo-950" : "border-zinc-300 dark:border-zinc-700"}`}><div className="flex flex-wrap justify-center gap-3"><label className={buttonSecondary}>Choose images<input ref={inputRef} type="file" multiple accept="image/png,image/jpeg,image/webp,image/gif" onChange={(event) => event.target.files && setChosenFiles(event.target.files)} className="sr-only" /></label><label className={buttonSecondary}>Choose folder<input ref={folderInputRef} type="file" multiple accept="image/png,image/jpeg,image/webp,image/gif" {...folderInputAttributes} onChange={(event) => event.target.files && setChosenFiles(event.target.files)} className="sr-only" /></label></div><p className="mt-3 text-xs text-zinc-400">Choose multiple files, choose an entire folder, or drag images from Explorer/Finder.</p>{files.length > 0 && <p className="mt-3 text-sm font-medium text-indigo-600">{files.length} image(s) ready</p>}</div>
    ...
    <section><div className="sticky top-14 z-30 mb-4 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-zinc-200 bg-white/95 p-3 shadow-sm backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/95"><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={allSelected} onChange={() => setSelected(allSelected ? new Set() : new Set(shots.map((shot) => shot.id)))} /> Select all {shots.length}</label><div className="flex flex-wrap items-center gap-2"><span className="text-xs text-zinc-500">{selectedCount} selected{selectedShots.length > 0 ? ` · ${selectedShots.filter((shot) => shot.sourceImagePath).length} with images` : ""}</span><button type="button" disabled={busy !== null || !selectedCount} onClick={() => bulkReview("approve")} className={buttonPrimary}>{busy === "approve" ? "Approving…" : "Approve selected"}</button><button type="button" disabled={busy !== null || !selectedCount} onClick={() => bulkReview("rework")} className={buttonSecondary}>{busy === "rework" ? "Updating…" : "Needs rework"}</button></div></div>
      {shots.length === 0 ? <EmptyState title="No shots yet" description="Return to the project and import your script first." /> : <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{shots.map((shot) => { const selectedTake = shot.takes.find((take) => take.isSelected) ?? shot.takes[0]; return <article key={shot.id} className={`${card} ${cardEnter} overflow-hidden`}><div className="relative aspect-video bg-zinc-100 dark:bg-zinc-950">{selectedTake ? <TakeMedia take={selectedTake} className="h-full w-full object-contain" /> : <div className="flex h-full w-full items-center justify-center text-sm text-zinc-400">No still uploaded</div>}<label className="absolute left-3 top-3 flex h-8 w-8 items-center justify-center rounded-lg bg-white/90 shadow dark:bg-zinc-900/90"><input type="checkbox" checked={selected.has(shot.id)} onChange={() => toggle(shot.id)} aria-label={`Select shot ${shot.order}`} /></label><span className={`${badge} absolute right-3 top-3 shadow ${SHOT_STATUS_STYLES[shot.status] ?? SHOT_STATUS_STYLES.PLANNED}`}>{shot.status.replaceAll("_", " ")}</span></div><div className="p-4"><div className="flex items-start justify-between gap-2"><div><h3 className="font-medium text-zinc-900 dark:text-zinc-50">{String(shot.order).padStart(2, "0")} · {shot.title ?? "Untitled"}</h3><p className="mt-1 text-xs text-zinc-500">{shot.takes.length} still take(s)</p></div><Link href={`/projects/${projectId}/shots/${shot.id}`} className="text-xs text-indigo-600 hover:underline">Open</Link></div>{shot.takes.length > 1 && <div className="mt-3 flex gap-2 overflow-x-auto pb-1">{shot.takes.map((take) => <button key={take.id} type="button" disabled={busy !== null} onClick={() => selectTake(take.id, shot.id)} className={`shrink-0 overflow-hidden rounded-lg border p-1 ${take.isSelected ? "border-indigo-500 ring-2 ring-indigo-500/20" : "border-zinc-200 dark:border-zinc-700"}`} title={`Select take ${take.versionNumber}`}><TakeMedia take={take} className="h-14 w-20 rounded object-cover" /><span className="block pt-1 text-[10px]">Take {take.versionNumber}</span></button>)}</div>}</div></article>; })}</div>}
    </section>
  </div>;
}
