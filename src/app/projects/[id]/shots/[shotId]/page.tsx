...
export default async function ShotDetail({ params }: PageProps<"/projects/[id]/shots/[shotId]">) {
  ...
  return <div className={`${pageShellNarrow} flex flex-col gap-8`}>
    ...
    {shot.sourceImagePath && <section className={cardPadded}><h2 className={`mb-3 ${sectionLabel}`}>Selected still image</h2><img src={shot.sourceImagePath.startsWith("/") ? shot.sourceImagePath : `/api/media/${shot.sourceImagePath.split("/").map(encodeURIComponent).join("/")}`} alt={shot.title ?? "Shot still"} className="max-h-96 w-full rounded-lg border border-zinc-200 object-contain dark:border-zinc-800" /></section>}
