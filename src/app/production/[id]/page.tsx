...
export default async function ProductionProjectPage({ params }: ProductionProjectPageProps) {
  ...
  if (!project) notFound();
  const serialized = { ...project, createdAt: project.createdAt.toISOString(), updatedAt: project.updatedAt.toISOString(), shots: project.shots.map((shot) => ({ ...shot, createdAt: shot.createdAt.toISOString(), updatedAt: shot.updatedAt.toISOString(), jobs: shot.jobs.map((job) => ({ ...job, createdAt: job.createdAt.toISOString(), completedAt: job.completedAt?.toISOString() ?? null, lastCheckedAt: job.lastCheckedAt?.toISOString() ?? null })) })) };
